...
function drawMeasurementLine(ctx: CanvasRenderingContext2D, a: Point, b: Point, isDraft: boolean) {
  const dist = Math.round(Math.hypot(b.x - a.x, b.y - a.y))
  const mid: Point = { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 }
  const lineColour = isDraft ? 'rgba(124,92,255,0.65)' : '#7c5cff'
  const labelColour = isDraft ? 'rgba(56,217,201,0.8)' : '#38d9c9'
  ...
  ctx.strokeStyle = isDraft ? 'rgba(124,92,255,0.35)' : 'rgba(124,92,255,0.6)'
  ctx.lineWidth = 1
  ctx.beginPath()
  ctx.roundRect(boxX, boxY, boxW, boxH, 4)
  ctx.stroke()
...
function drawCornerIndicator(ctx: CanvasRenderingContext2D, corner: Point, isSnapTarget: boolean) {
  const colour = isSnapTarget ? '#38d9c9' : 'rgba(56,217,201,0.5)'
  ...
  ctx.strokeRect(corner.x - squareHalf, corner.y - squareHalf, squareHalf * 2, squareHalf * 2)
...
function drawFrame(
  ...
  for (const corner of snappableCorners) {
    const isSnap = snapTarget && Math.abs(corner.x - snapTarget.x) < 0.5 && Math.abs(corner.y - snapTarget.y) < 0.5
    if (!isSnap) drawCornerIndicator(ctx, corner, false)
  }
...
// elementFromPoint correctly skips pointer-events:none elements (our canvas)
function isCornerVisible(x: number, y: number, el: Element): boolean {
  if (x < 0 || y < 0 || x > window.innerWidth || y > window.innerHeight) return false
  const top = document.elementFromPoint(x, y)
  return top ? (top === el || el.contains(top)) : false
}
...
function computeSnapState(cursor: Point) {
  ...
  for (const el of document.querySelectorAll('*')) {
    ...
    for (const corner of corners) {
      const d = Math.hypot(corner.x - cursor.x, corner.y - cursor.y)
      if (d <= INDICATOR_RADIUS && d < nearestDist && isCornerVisible(corner.x, corner.y, el)) {
        nearestDist = d
        nearest = corner
      }
    }
    ...
    if (nearest) {
      ...
      if (nearestDist <= SNAP_RADIUS && nearestDist < snapDist) {
  ...
  return { snappableCorners, snapTarget }
}
...
export function PixelPolice() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      drawFrame(ctx, measurements, pendingAnchor, cursor, snappableCorners, snapTarget)
  ...
  const handleMeasurementClick = useCallback((e: MouseEvent) => {
    ...
    // Determine target on click directly to avoid needing it in state
    const { snapTarget } = computeSnapState({ x: e.clientX, y: e.clientY })
    const point: Point = snapTarget ?? { x: e.clientX, y: e.clientY }
  ...
  useEffect(() => {
    if (!active) return
    document.addEventListener('click', handleMeasurementClick, { capture: true })
    return () => document.removeEventListener('click', handleMeasurementClick, { capture: true })
